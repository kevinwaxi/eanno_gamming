            ...
            <div class="modal-body" v-show="!detailMode">
              ...
              <div class="row mt-3">
                <div class="col-12">
                  ...
                  <Select v-model="form.type_id" multiple :max-tag-count="3">
                    <Option v-for="(type, i) in types.data" :value="type.id" :key="i">
                      {{ type.name }}
                    </Option>
                  </Select>
                </div>
              ...
              <div class="row mt-3">
                ...
                <Upload
                  ...
                  ref="uploads"
                  type="drag"
                  action="/api/v1/admin/settings/inventories/games/upload"
                  :headers="{
                    'x-csrf-token': token,
                ...
                <div class="demo-upload-list" v-if="form.cover_image">
                  ...
                  <div class="demo-upload-list-cover">
                    ...
                    ></Icon>
                    <Icon v-else type="ios-trash-outline" @click="deleteImage"></Icon>
                  </div>
                </div>
              </div>
              <div class="row mt-3">
...
export default {
  ...
  methods: {
    ...
    async getGames(page = 1) {
      const res = await this.callApi(
        "get",
        `/api/v1/admin/settings/inventories/games?page=${page}
        &total=${this.total}
        &q=${this.search}
    ...
    async createGame() {
      ...
      const res = await this.callApi(
        "post",
        "/api/v1/admin/settings/inventories/games",
        this.form
      );
      if (res.status === 200) {
    ...
    async updateGame(game) {
      ...
      const res = await this.callApi(
        "put",
        `/api/v1/admin/settings/inventories/games/${game}`,
        this.form
      );
      if (res.status == 200) {
        this.closeModal();
    ...
    async deleteGame(game) {
      ...
      if (res.status == 204) {
        this.w("Game deleted");
        this.checked = this.checked.filter((id) => id != game);
    ...
    async deleteImage(editMode = false) {
      ...
      const res = await this.callApi(
        "post",
        "/api/v1/admin/settings/inventories/games/deleteCover",
        {
          image_name: image,
        }
      );
    ...
    handleSuccess(res, file) {
      if (this.isEditingItem) {
        return (this.form.cover_image = `/uploads/games/icons/${res}`);
      }
      res = `/uploads/games/icons/${res}`;
