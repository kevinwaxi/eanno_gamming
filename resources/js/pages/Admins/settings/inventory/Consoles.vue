            ...
            <div class="card-footer pt-0 p-3 d-flex align-items-center">
              <div class="w-60">
                <p class="text-sm" v-if="consoles.data">
                  Showing
                  <b>{{ consoles.meta.from }}</b> to <b>{{ consoles.meta.to }}</b> out of
...
export default {
  ...
  methods: {
    ...
    async getConsoles(page = 1) {
      const res = await this.callApi(
        "get",
        `/api/v1/admin/settings/inventories/consoles?page=${page}
        &total=${this.total}
        &q=${this.search}
    ...
    async updateConsole(console_id) {
      this.processing = true;
      const res = await this.callApi("put", `/api/v1/consoles/${console_id}`, this.form);
      if (res.status == 200) {
        this.closeModal();
        this.getConsoles();
