...
export default {
  ...
  methods: {
    ...
    async selectAllRecords() {
      const res = await this.callApi('get', '/api/v1/roles/all/')
      if (res.status === 200) {
    ...
    async getPermissions() {
      const res = await this.callApi('get', '/api/v1/permissions')
      if (res.status == 200) {
    ...
    async createRole() {
      ...
      const res = await this.callApi('post', '/api/v1/roles', this.form)
      if (res.status === 201) {
    ...
    async deleteRecords(checked) {
      const res = await this.callApi(
        'delete',
        `/api/v1/roles/massDestroy/${checked}`
      )
      if (res.status === 204) {
        this.s('Role deleted successfully')
